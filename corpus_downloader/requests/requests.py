...
@retry(wait_exponential_multiplier=1000, stop_max_delay=60)
def put(url: str, payload: dict, auth: Auth) -> Response:
    """Wrapper to the request PUT to handle log and retry

    :param url: URL to call
    :type url: str
    :param auth: Auth Object to use when requesting URL
    :type auth: Auth
    :param payload: Put Payload to send
    :type payload: dict
    :raises err: HTTPError
    :return: Request Response Object
    :rtype: Response
    ...
    url = quote(url, safe='/:')
    try:
        res = requests.put(url, json=payload, headers={'Authorization': 'Bearer {}'.format(auth.jwt)}, timeout=20)
        res.raise_for_status()
    except HTTPError as err:
        LOGGER.error("request returned {}".format(err.response.status_code))
        LOGGER.error(err.request.url, err.response.text)
